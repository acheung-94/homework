...
def pulp_fiction_actors
  ...
  Actor
    .select(:id, :name)
    .joins(:movies) #use the pre-defined hasmanythrough method instead of using 2 separate methodss!
...
def uma_movies
  ...
  # hint: use 'select', 'joins', 'where', and 'order' start on castings table? 
  Movie
    .select(:id, :title, :yr)
    .joins(:actors)
